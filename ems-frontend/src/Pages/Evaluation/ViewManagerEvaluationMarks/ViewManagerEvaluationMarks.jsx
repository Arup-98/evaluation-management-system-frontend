...
const ViewManagerEvaluationMarks = () => {
  ...
  const handleBatchChange = (event) => {
    ...
    axios.get(`http://localhost:8088/managerEvaluation/allMarks`)
      .then((response) => {
            ...
            <tbody>
              {marksData.map((mark) => (
                <tr key={mark.id}>
                  ...
                  <td>{mark.commonScore}</td>
                  <td>{mark.officeRules}</td>
                  <td>{mark.sincerity}</td>
                  <td>{mark.qualityMindset}</td>
                  <td>{mark.attendance}</td>
                  <td>{mark.communicationSkill}</td>
                  <td>{mark.englishLanguageSkill}</td>
